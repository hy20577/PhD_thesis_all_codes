...
% lag in terms of second.
% Adj --> Adjacency Matrix of the initial Network

function [tvalues, LYA, Orbits] = lag_Lorenz_generator(x0, t0, tf, k, coupling_strength, sigma, rho, beta, lag, Adj, time_window, std_threshold, max_discard)
...
lag_step = lag/stept;
            ...
            for i=1:lag_step   % Generate first a few trajectory for lag computation            
                tspan= [t t+stept];
                ...
                Y = Y(end,:);
                y0(1:n) = Y(1:n);
                discarded_orbits(i,:) = y0(1:n);
            end

y0_lag = zeros(1,n^2+n);
y0_lag(1:n) = discarded_orbits(1, 1:n);
for i=1:n
    y0_lag((n+1)*i) = 1; % initial deviation vectors: unit vectors.
end
...
for i = 1:time_window
        ...
        tspan= [t t+stept];
        options = odeset('RelTol',1e-8);
        [T, Y] = ode45(@(t,y) lorenz_ext(t,y, Adj, coupling_strength, sigma, rho, beta), tspan, y0_lag, options);
        ...
        discarded_orbits(lag_step+i,:) = y0(1:n);
        y0_lag(1:n) = discarded_orbits(i+1,:);
        y0_lag(n+1:n^2+n) = reshape(NK,1,n^2);
...
sum2 =0;
 while std(LYA(end-time_window+1:end,1)) > std_threshold 
        ...
        tspan= [t t+stept];
        options = odeset('RelTol',1e-8);
        [T, Y] = ode45(@(t,y) lorenz_ext(t,y, Adj, coupling_strength, sigma, rho, beta, xlag), tspan, y0, options);
        ...
        discarded_orbits(time_window+sum2,:) = y0;
