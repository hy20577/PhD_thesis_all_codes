function [tvalues, Orbits, LYA] = RK4(extended_ode, t0, x0, tf, dt, varargin)
...
f = waitbar(0,'1');
for i = 1:k
    k1 = extended_ode(t,         y0,  varargin{:});
    k2 = extended_ode(t+dt/2,    y0+k1*dt/2,  varargin{:});
    k3 = extended_ode(t+dt/2,    y0+k2*dt/2,  varargin{:});
    k4 = extended_ode(t+dt,      y0+k3*dt,  varargin{:});
  ...
  waitbar(i/k, f, sprintf('approximately %.2f secs remain',((k-i)/i)*toc(start_time)))
end
end
