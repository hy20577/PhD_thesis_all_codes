function [Orbits, Lya_overtime] = Log_generator(Adjacency, alpha, x0, r, niter, time_window, std_threshold, max_discard, savefigs)
...
Orbits = zeros(niter, k);
    ...
    for i = 1:time_window
        K = zeros(k,k);   % matrix stores deviation vectors in columns.
            J = Jacob_log(v, r, Adjacency, alpha); 
...
Jacob_log = zeros(k,k);
    for i=1:k
        for j=1:k
                ...
                if i == j
                    Jacob_log(i,j) = df_log(r,x(i))*(1-alpha);
                else
                    if sum(A(i,:)) ~= 0
...
clf = zeros(M,1);
    ...
    for i=1:M %rows loop
        ...
        clf(i) = logis(r, x(i)) * (1-alpha) + coupling_term;
    end
